...
**技术实现方向**：文本会话依然复用 Realtime WebSocket 这套协议机制（不是切换成一次性 HTTP 补全调用）——原因是现有的记忆检索/注入逻辑（`groundAndRespond`/`handleUserTurn`：查记忆→`session.update` patch instructions→`response.create`）是深度绑定在 Realtime 协议上的，改用无状态的 HTTP 补全调用意味着要把这套逻辑重写一遍（每次请求都要带上完整上下文，因为没有"会话"可以 patch 了），成本远大于收益。文本会话的连接只是**不接麦克风、不采集音频**，并且把 `session.update.modalities` 设成只有 `["text"]`（不含 `"audio"`），让服务端从会话层面就不生成语音——这是**待验证的实现细节**，不是已经确认可行的结论：`modalities` 这个字段本身已经在这个 App 里反复用过（`["text","audio"]`），但"只设 `["text"]` 能不能让服务端完全不生成音频、协议其余部分是否照常工作"没有实测过，动手时要先验证，不能假设它就是理所当然生效的。
...
1. 文本会话/语音会话状态机拆分（web 先做，验证 `modalities: ["text"]` 能不能让服务端不生成语音，验证口述"直接发"过户到文本会话正确工作），再port到 iOS
