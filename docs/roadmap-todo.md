...
- [x] **拆成两套独立的连接/状态机（web 端已完成，2026-08-23）**：文本会话（打字+口述转文字，彻底不碰麦克风、不碰 TTS，回复只要文字）跟语音会话（现有实时对话，不变）之前共用同一套 `ws`/`STATE`，`sendTextMessage()` 在空闲时会调 `start()`，导致打字发消息也会打开麦克风、语音播放回复——已确认属实、已修复。web 端拆成 `ws`/`STATE`（语音）+ `textWs`/`TEXT_STATE`（文本）两套独立连接持有者，`handleUserTurn`/`sendSessionUpdateAndWait` 改成按 `session`（`voiceSession`/`textSession`）参数化，避免复制一份记忆检索/grounding 逻辑。**协议细节已实测**（不是假设）：直接连真实 `qwen3.5-omni-flash-realtime` 测过 `session.update.modalities: ["text"]`——服务端完全不生成任何音频相关事件（`got_audio_event: False`），回复通过 `response.text.delta`/`response.text.done` 这对全新事件（跟语音会话用的 `response.audio_transcript.*`不是同一对）返回。**加了一条之前没有的规则：三种输入方式（语音会话/文本会话/口述）互斥，同一时刻只能有一个在活动**——不只是 UX 顺手，是因为文本会话和语音会话共用同一份 `assistantBubbleEl`/`assistantHasDelta`（气泡流式拼接状态），两边同时有回复在流式返回会互相污染，互斥是最简单正确的解法。**Playwright 端到端验证过**：打字发消息全程不碰麦克风（`micStream`/`captureCtx` 保持 null）、`state` 不受影响、拿到纯文字回复；语音会话本身（含打断）完全不受影响，回归测过；三种输入方式互斥的按钮禁用状态验证过。iOS 端还没 port，是这个阶段接下来要做的。
- [x] **口述"直接发"改指向文本会话（web 端已完成，2026-08-23）**：`promoteDictationConnection` 重命名为 `promoteDictationConnectionToTextSession`，现在过户给 `textWs` 而不是 `ws`。顺带把 `startDictation()` 自己的 `session.update` 也改成了 `modalities: ["text"]`（口述从来不生成回复，跟文本会话本来就是同一种配置），这样过户不需要再改 modalities，只需要一次 `instructions` patch——比之前"过户给语音会话"更简单，不是更复杂。**Playwright 验证过**：口述"直接发"全程不碰麦克风、正确过户进 `textWs`、`textState` 变成 `ready`、拿到文字回复；口述"编辑后发"路径不受影响（仍然正常关闭连接、回填输入框）。
