...
- [ ] **拆成两套独立的连接/状态机**：文本会话（打字+口述转文字，彻底不碰麦克风、不碰 TTS，回复只要文字）跟语音会话（现有实时对话，不变）目前共用同一套 `ws`/`ConversationViewModel`，`sendTextMessage()`/`sendText(_:)` 在空闲时会调 `start()`，导致打字发消息也会打开麦克风、语音播放回复——这是确认属实的真 bug，不是设计选择。拆成两个独立的连接持有者，各自状态机。**待验证**：文本会话连接把 `session.update.modalities` 设成只有 `["text"]` 能不能让服务端完全不生成音频——这个字段本身用过，但"只设 text 会不会有其他副作用"没实测过。web 先做，验证完再 port iOS。
- [ ] **口述"直接发"改指向文本会话**：`promoteDictationConnection`/`sendText(_:reusing:)` 现在过户给语音会话，跟新设计矛盾，应该过户给文本会话——口述连接本来就没有"持续开麦、语音回复"这些语义，过户给文本会话反而更贴合，不是额外负担。
