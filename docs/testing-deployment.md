...
4. 模型/音色已经预填好实测过的默认值（`qwen3.5-omni-flash-realtime` + `Serena`），音色可以在设置页的下拉菜单里换成另外 5 个测过能用的选项（Tina/Sunnybobi/Ethan/Raymond/Dylan）；一般不用改地址，除非哪天这些值又变了（Qwen 这条产品线迭代较快，建议先看上面那份配置指南文档）。
5. 点麦克风开始对话，点"记忆"图标（左上角大脑）能看到 App 目前记住的内容。
6. **口述转文字**（新功能，2026-08-23）：不想打字时，点输入框旁边的麦克风图标（不是屏幕上那个大的语音对话按钮），进入录音状态，输入框那一行会换成 X / 状态文字 / ■ / ↑ 四个控件：
   - **X**：取消，什么都不发生
   - **■**：停止录音，AI 整理成书面文字后**回填到输入框**，可以看、可以改，改完自己手动点发送
   - **↑**：停止录音，AI 整理后**直接发送**，不用二次确认

   这条链路（语音识别 → AI 整理 → 发送/回填）在 iOS 上是这次改动里最新、改得最多、也最没经过验证的部分——尤其是"直接发送"这条路径，为了省一次重新连接的开销，做了"把口述用的连接直接转手给对话逻辑复用"这个优化，涉及在两个 ViewModel 之间转移 `RealtimeClient` 实例的所有权。第一次在 Mac 上编译，这块建议重点测：录一段话分别走 ■ 和 ↑ 两条路径，确认文字整理得对、"直接发"之后助手真的用语音回复了（不是连上了但连接其实是死的）。详细设计见 `docs/app-design.md` 第三节。
...
| 口述转文字点了 ■/↑ 之后"整理中…"一直转，最后失败 | 大概率是 AI 整理这一步（`qwen-turbo`，走 HTTP 而不是 WebSocket）超时了——正常应该在几秒内，超过 60 秒服务端会主动返回超时，客户端还有 65 秒的兜底；如果经常超时，先确认这个 API Key 有没有 `qwen-turbo`（普通文本模型）的调用权限，跟 Realtime 语音模型不是同一个权限范围 |
| 口述"直接发"之后没反应、也没报错 | 这条路径复用了口述录音时的连接而不是重新连一条，如果复用逻辑本身有问题（这块完全没编译验证过，见下面的诚实说明），可能会卡在"连接中"或者静默失败；先看有没有报错提示，没有的话检查 `ConversationViewModel.sendText(_:reusing:)` 和 `DictationViewModel.finishForDirectSend()` 这两处 |
...
3. **iOS 的 Swift 源码本身**——只做过手工数括号配对，确认没有明显语法失衡，**没有做过真正的编译检查**，SwiftUI 的具体写法（`Picker`、`Form`/`Section` 嵌套、新加的 `RealtimeConfigStore.effectiveWorkspaceURL` 等静态方法调用）有没有问题完全未知。这里面**口述转文字功能是风险最高的一块**（`DictationViewModel`、`DictationCleanupClient`、以及 `ConversationViewModel.sendText(_:reusing:)` 里"直接发"的连接复用逻辑）——不光没编译过，逻辑本身也是这次改动里最新、改动量最大的，第一次编译遇到问题大概率就在这几个文件里。
...
- [ ] 检查 App 里有没有开发调试用的东西不该带到生产版本（比如 Mock 数据、日志里会不会打印敏感信息）——音色列表已经精简过了（6 个测过的选项，不是早期一大串），这条不用再做
...
- 口述转文字全程（真实说话 → 识别 → AI 整理 → 回填/直接发送），尤其是"直接发"路径的连接复用——网页版用假麦克风 + 注入模拟转写文本测过链路本身能跑通，但没有真实语音，iOS 端连假麦克风测试都没做过
