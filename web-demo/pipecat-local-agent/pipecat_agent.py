...
async def entrypoint(room_url: str, token: str, room_name: str, config: LocalAgentConfig):
    ...
    worker = PipelineWorker(
        pipeline,
        # SenseVoice model load + LanceDB init + LiveKit connect + dashscope
        # TTS websocket handshake can push total setup time past the default
        # 20s budget. Bump to 60s so cold-start sessions don't self-destruct.
    ...
    async def on_disconnected(_transport):
        logger.warning("LiveKit transport disconnected — stopping runner so main loop can reconnect")
        # ``runner.run()`` is an infinite await; without stopping the runner,
        # main() never gets back to the ``while True`` reconnect loop and the
        # agent process becomes a zombie (still listening on no sockets,
        # never joining the room again). Stop the runner so entrypoint()
        # returns and main() can spin up a fresh session.
        try:
            await runner.stop()
        except Exception as exc:
            logger.warning(f"runner.stop() raised (continuing): {exc}")
